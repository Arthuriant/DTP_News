...
const SingleItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group h-full flex flex-col pt-2 pb-2">
      {/* Container Kartu Keseluruhan - Timbul dengan warna Putih & Shadow Bawaan */}
      <div className="relative flex flex-col h-full rounded-[1.5rem] bg-white border border-[#E5D7C1] shadow-[0_15px_40px_-10px_rgba(45,26,17,0.1)] transition-all duration-[800ms] group-hover:shadow-[0_30px_60px_-15px_rgba(197,160,89,0.3)] group-hover:border-[#C5A059]/50 overflow-hidden">
        
        {/* Bingkai Dalam Ala Lukisan (Inner Frame) */}
        <div className="absolute inset-2 border border-[#E5D7C1]/40 rounded-xl pointer-events-none z-30 transition-colors duration-[800ms] group-hover:border-[#C5A059]/30"></div>

        {/* ================= BAGIAN ATAS: ETALASE GAMBAR ================= */}
        <div className="relative w-full aspect-[4/5] sm:aspect-square flex items-center justify-center overflow-hidden bg-gradient-to-b from-[#F8F3E9]/50 to-white">
          
          {/* BAGIAN DIUBAH: Tekstur Batik dinaikkan menjadi 10% (opacity-10) */}
          <div 
          ...
          {/* Efek Cahaya Etalase saat Hover */}
          <div className="absolute inset-0 bg-[#C5A059] blur-[80px] opacity-0 group-hover:opacity-[0.12] transition-opacity duration-[1000ms] rounded-full w-3/4 h-3/4 m-auto pointer-events-none z-0"></div>
          ...
          <Image 
            ...
            width={280} 
            height={280} 
            className="object-contain relative z-10 drop-shadow-[0_15px_25px_rgba(45,26,17,0.15)] transform transition-transform duration-[1200ms] ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:scale-110"
          />

          {/* ================= TOMBOL AKSI KACA (GLASSMORPHISM) ================= */}
          <div className="absolute right-4 bottom-4 translate-x-[150%] flex flex-col gap-2.5 ease-[cubic-bezier(0.25,1,0.5,1)] duration-700 group-hover:translate-x-0 z-40">
            ...
            <button
              ...
              title="Lihat Detail"
              className="flex items-center justify-center w-10 h-10 rounded-full shadow-[0_8px_20px_rgba(45,26,17,0.15)] backdrop-blur-md bg-white/80 border border-white/50 text-[#2D1A11] transition-all duration-300 hover:bg-[#2D1A11] hover:text-[#C5A059] hover:border-[#2D1A11] hover:-translate-x-1"
            >
        ...
        {/* ================= BAGIAN BAWAH: DESKRIPSI & HARGA ================= */}
        <div className="flex flex-col flex-grow justify-center text-center p-6 border-t border-[#E5D7C1]/30 bg-white relative z-20">
          
          {/* Bintang & Review */}
          <div className="flex items-center justify-center gap-2 mb-3">
            <div className="flex items-center gap-0.5">
              {[...Array(5)].map((_, index) => (
                <svg key={index} className="w-3 h-3 text-[#C5A059]" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                </svg>
              ))}
            </div>
            <span className="font-sans text-[9px] text-[#C5A059] font-bold tracking-[0.2em] uppercase mt-0.5">({item.reviews} Ulasan)</span>
          </div>

          {/* Judul */}
          <h3 className="font-serif text-xl text-[#2D1A11] ease-out duration-300 group-hover:text-[#C5A059] mb-4 leading-snug">
            <Link href="/shop-details"> {item.title} </Link>
          </h3>

          {/* Harga */}
          <div className="flex flex-col items-center justify-center gap-1">
            <span className="font-serif font-medium tracking-wide text-2xl text-[#2D1A11]">
              Rp {item.discountedPrice.toLocaleString('id-ID')}
            </span>
            {item.price > item.discountedPrice && (
              <span className="font-sans font-medium text-[11px] tracking-widest text-[#C5A059] line-through uppercase">
