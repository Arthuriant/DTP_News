...
export default function BagCustomizer() {
  ...
  // STATE BARU: Untuk mengatur visibilitas (nyala/mati) tiap part
  ...
  // FUNGSI BARU: Untuk mematikan/menyalakan kompartemen
            ...
            <div className="w-full lg:w-[55%] p-6 lg:p-10 border-b lg:border-b-0 lg:border-r border-gray-100 relative flex flex-col bg-[#f8f9fa] overflow-hidden">
              ...
              <div className="flex-grow flex flex-col items-center justify-center w-full relative z-10 pt-4">
                ...
                {activeView === "front" && (
                  <div key="front" className="animate-soft-fade w-full max-w-[420px] aspect-[4/5] relative">
                    {visibleParts.tali && <DynamicPart pov="Front" partName="tali" color={selections.tali} texture={textureSelections.tali} zIndex={10} />}
                    {visibleParts.telinga && <DynamicPart pov="Front" partName="telinga" color={selections.telinga} texture={textureSelections.telinga} zIndex={20} />}
                ...
                {activeView === "back" && (
                  <div key="back" className="animate-soft-fade w-full max-w-[420px] aspect-[4/5] relative">
                    {visibleParts.tali && <DynamicPart pov="Back" partName="tali" color={selections.tali} texture={textureSelections.tali} zIndex={10} />}
                    {visibleParts.telinga && <DynamicPart pov="Back" partName="telinga" color={selections.telinga} texture={textureSelections.telinga} zIndex={30} />}
                    {visibleParts.body && <DynamicPart pov="Back" partName="body" color={selections.body} texture={textureSelections.body} zIndex={20} />}
                  </div>
                )}
                ...
                {activeView === "top" && (
                  <div key="top" className="animate-soft-fade w-full max-w-[420px] aspect-[4/5] relative">
                    {visibleParts.tali && <DynamicPart pov="Top" partName="tali" color={selections.tali} texture={textureSelections.tali} zIndex={10} />}
                    {visibleParts.telinga && <DynamicPart pov="Top" partName="telinga" color={selections.telinga} texture={textureSelections.telinga} zIndex={30} />}
                    {visibleParts.body && <DynamicPart pov="Top" partName="body" color={selections.body} texture={textureSelections.body} zIndex={20} />}
                  </div>
                )}
                ...
                {activeView === "360" && (
                  ...
                  >
                    <div className="absolute inset-0 pointer-events-none">
                      {visibleParts.telinga && (
                        <SpritePart partName="telinga" color={selections.telinga} texture={textureSelections.telinga} zIndex={20} currentFrame={frame360} totalFrames={TOTAL_FRAMES} />
                      )}
                      
                      {visibleParts.body && (
                        <SpritePart partName="body" color={selections.body} texture={textureSelections.body} zIndex={30} currentFrame={frame360} totalFrames={TOTAL_FRAMES} />
                      )}
                      
                      {visibleParts.tali && (
                        <SpritePart partName="tali" color={selections.tali} texture={textureSelections.tali} zIndex={10} currentFrame={frame360} totalFrames={TOTAL_FRAMES} />
                      )}
                      
                      {/* {visibleParts.body && (frame360 < 4 || frame360 > 12) && (
                        <>
                          <StaticSpritePart imageUrl="/assets/TasKelalawar/360/mata.png" zIndex={40} currentFrame={frame360} totalFrames={TOTAL_FRAMES} />
                          <StaticSpritePart imageUrl="/assets/TasKelalawar/360/gigi.png" zIndex={60} currentFrame={frame360} totalFrames={TOTAL_FRAMES} />
                        </>
                      )} */}
                    </div>
              ...
              <div className="mt-6 flex justify-center z-20 pb-2">
                <div className="flex items-center gap-3 bg-[#f8f9fa] px-4 py-2.5 rounded-2xl border border-[#cbd5e1] shadow-sm">
                  {/* ... (Tombol panah dan thumbnail tidak berubah) ... */}
            ...
            <div className="w-full lg:w-[45%] flex flex-col h-full bg-white">
              <div className="px-8 lg:px-12 pt-10 pb-6 border-b border-gray-100">
                ...
                <p className="text-[#6b7280] text-[15px] leading-relaxed">
                  Sesuaikan warna dan material, atau lepaskan bagian yang tidak diinginkan.
                </p>
              </div>
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3 font-bold text-[#111827] text-[13px] uppercase tracking-wider">
                          ...
                          <button
                            ...
                            className={`relative inline-flex h-6 w-12 shrink-0 items-center rounded-full transition-colors duration-300 focus:outline-none 
                              ${isVisible ? 'bg-[#4154f1]' : 'bg-[#cbd5e1 border border-gray-300 shadow-inner]'}`}
                            title={isVisible ? "Sembunyikan bagian ini" : "Tampilkan bagian ini"}
                          >
                            <span 
                        ...
                        <span className={`text-[13px] font-medium ${isVisible ? 'text-gray-500' : 'text-red-400'}`}>
                          {isVisible ? activeColorName : "Dilepas"}
                        </span>
                      </div>
                            ...
                            {textureOptions.map((tex) => {
                              ...
                              return (
                                <button
                                  ...
                                  disabled={isDisabled}
                                  title={isDisabled ? "Material ini belum tersedia" : ""}
