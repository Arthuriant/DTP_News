...
import StaticPart from "../Proto/StaticPart";
import Breadcrumb from "../Common/Breadcrumb";
import SpritePart from "../Proto/SpritePart";
import StaticSpritePart from "../Proto/StaticSpritePart";
import { PRODUCTS_CONFIG, ProductConfig } from "@/config/products";

// --- KOMPONEN WRAPPER ---
export default function BagCustomizer({ productId = "tas_mini" }: { productId?: string }) {
...
function BagCustomizerInner({ product }: { product: ProductConfig }) {
 ...
 const renderProductParts = (pov: string) => {
  ...
  const activeKompartemen = shapeSelections["kompartemen"];
  const bodyTexture = textureSelections["body"] || "base";
  const bodyColor = selections["body"];
  ...
  return (
    <>
      ...
      <StaticPart
        key="top-tali"
        imageUrl={`/assets/products/${product.id}/top/tali-base.png`}
        zIndex={5}
        altText="Tali Top"
      />

      {/* KOMPARTEMEN STATIC */}
      {activeKompartemen === "kancing" ? (
        <StaticPart
          key="top-kancing"
          imageUrl={`/assets/products/${product.id}/top/kancing-base.png`}
          zIndex={10}
          altText="Kancing Top"
        />
      ) : (
        <StaticPart
          key="top-pengait"
          imageUrl={`/assets/products/${product.id}/top/pengait-base.png`}
          zIndex={10}
          altText="Pengait Top"
        />
      )}
  ...
  const penutupTexture = textureSelections["penutup"] || "base";
  const penutupColor = selections["penutup"];
  ...
  return (
    <>
      ...
      {isPenutupVisible && (
        <SpritePart
          productId={product.id}
          partName="trim-penutup"
          color={selections["trim-penutup"]}
          texture="base"
          zIndex={30}
          currentFrame={frame360}
          totalFrames={17}
      ...
      {activeKompartemen === "pengait" && (
        <>
          ...
          {/* pengait3 hanya jika penutup ON */}
          {isPenutupVisible && (
            <StaticSpritePart
    ...
    if (pov === "Back") {
      ...
      // Jangan render kompartemen & pengait2 di back
      if (part.id === "kompartemen") return null;
      if (part.id === "pengait2") return null;

      // trim-penutup ikut hilang jika penutup off
      if (part.id === "trim-penutup" && !isPenutupVisible) {
        return null;
      }
      ...
      const activeTexture = textureSelections[part.id] || "base";
    ...
    // KOMPARTEMEN (STATIC)
    // =========================
    if (part.id === "kompartemen") {

  const activeVariant = part.variants?.find(
    v => v.id === activeKompartemen
  );

  return activeVariant?.staticOverlays?.map(overlay => {

    // =========================
    // KANCING MODE
    // =========================
    if (activeKompartemen === "kancing") {

      // 🔥 kancing3 selalu muncul
      if (overlay.id === "kancing3") {
        return (
          <StaticPart
            key={`${pov}-${overlay.id}`}
      ...
      // 🔥 Jika penutup ON → tampilkan kancing2
      if (overlay.id === "kancing2" && isPenutupVisible) {
        return (
          <StaticPart
            key={`${pov}-${overlay.id}`}
            imageUrl={overlay.url}
      ...
      // 🔥 Jika penutup OFF → tampilkan kancing1
      if (overlay.id === "kancing1" && !isPenutupVisible) {
        return (
          <StaticPart
            key={`${pov}-${overlay.id}`}
            imageUrl={overlay.url}
    ...
    // PENGAIT MODE
    // =========================
    if (activeKompartemen === "pengait") {

      // pengait1 selalu muncul
      if (overlay.id === "pengait1") {
        return (
          <StaticPart
            key={`${pov}-${overlay.id}`}
            imageUrl={overlay.url}
      ...
      // pengait3 hanya muncul jika penutup visible
      if (overlay.id === "pengait3" && isPenutupVisible) {
        return (
          <StaticPart
            key={`${pov}-${overlay.id}`}
            imageUrl={overlay.url}
    ...
    if (part.id === "pengait2") {
      ...
      // Hide jika:
      // - penutup dimatikan
      // - kompartemen bukan pengait
      if (!isPenutupVisible || activeKompartemen !== "pengait") {
        return null;
      }

      // 🔥 IKUT PENUTUP
      const penutupColor = selections["penutup"];
      const penutupTexture = textureSelections["penutup"];
  ...
  <>
    <StaticPart
      imageUrl={`/assets/products/${product.id}/back/tali-base.png`}
      zIndex={30}
      altText="Tali Belakang"
    />
    <StaticPart
      imageUrl={`/assets/products/${product.id}/back/kancing-base.png`}
      zIndex={5}
      altText="Kancing Belakang"
    />
  </>
)}
                  </div>
